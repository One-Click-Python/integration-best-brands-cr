...
async def get_shopify_products() -> Set[str]:
    ...
    try:
        ...
        while has_next_page:
            ...
            response = await shopify_client.product_client.execute_query(query, variables)
...
async def analyze_zero_stock_products():
    ...
    try:
        async with rms_handler.conn_db.get_session() as session:
            ...
            # 1. Obtener productos sin stock que fueron modificados recientemente
            # Nota: Ajusta el campo de fecha según tu base de datos
            query_recent = """
            ...
            AND Price > 0
            AND Quantity = 0
            -- AND LastUpdated > DATEADD(day, -30, GETDATE())  -- Últimos 30 días
            """
            
            # 2. Categorías de productos sin stock
            query_categories = """
