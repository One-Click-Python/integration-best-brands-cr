...
from app.services.rms_to_shopify import RMSToShopifySyncOrchestrator
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Sincronizar un CCOD específico desde RMS a Shopify")
    parser.add_argument("ccod", type=str, help="El CCOD del producto a sincronizar (ej: 27DL81)")
    parser.add_argument("--no-force", action="store_true", help="No forzar actualización si el producto ya existe")

    args = parser.parse_args()

    # Ejecutar sincronización
    asyncio.run(sync_specific_ccod(ccod=args.ccod.upper(), force_update=not args.no_force))
