...
class CriticalProductSync:
    ...
    async def check_product_exists_in_shopify(self, ccod: str) -> bool:
        ...
        try:
            ...
            response = await self.shopify_client.product_client.execute_query(query, variables)
    ...
    async def sync_single_product(self, ccod: str, force_update: bool = False):
        ...
        try:
            ...
            async with self.rms_handler.conn_db.get_session() as session:
                ...
                # Sincronizar con Shopify usando ProductProcessor
                success, error = await self.product_processor.process_single_product(product_data)
                
                if success:
                    logger.info(f"✅ Producto {ccod} sincronizado exitosamente")
                    self.stats['synced'] += 1
                else:
                    logger.error(f"❌ Error sincronizando producto {ccod}: {error}")
                    self.stats['errors'] += 1
...
async def main():
    ...
    parser.add_argument('--mode', choices=['negative', 'sale', 'file'], required=True,
