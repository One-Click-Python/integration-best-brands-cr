...
async def get_critical_products(query_executor: QueryExecutor, limit: int = 10) -> List[dict]:
...
async def update_shopify_inventory(shopify_client: ShopifyGraphQLClient, sku: str, quantity: int) -> bool:
    ...
    try:
        ...
        result = await shopify_client._execute_query(mutation, update_input)
...
async def sync_critical_product(query_executor: QueryExecutor, shopify_client: ShopifyGraphQLClient, ccod: str) -> dict:
...
async def main():
    ...
    try:
        ...
        critical_products = await get_critical_products(query_executor, LIMIT)
        ...
        for product in critical_products:
            product_stats = await sync_critical_product(query_executor, shopify_client, product["ccod"])
        ...
        success_rate = (
            (total_stats["updated"] / (total_stats["updated"] + total_stats["errors"]) * 100)
            if (total_stats["updated"] + total_stats["errors"]) > 0
            else 0
        )
