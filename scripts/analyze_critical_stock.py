...
async def get_critical_products_from_rms(rms_handler: RMSHandler) -> Dict:
    ...
    async with rms_handler.conn_db.get_session() as session:
        ...
        return {
            ...
            'sale_zero_stock': sale_zero_stock,
            'recently_out': recently_out
        }
...
async def analyze_and_recommend():
    ...
    try:
        ...
        critical_products = await get_critical_products_from_rms(rms_handler)
        ...
        if critical_products['recently_out']:
            for i, row in enumerate(critical_products['recently_out'][:10], 1):
                print(f"{i:2}. CCOD: {row.CCOD:15s} | Tallas sin stock: {row.SizesOutOfStock:2d} | {row.Description[:30]:30s}")
