...
class CompleteSyncManager:
    ...
    async def get_all_ccods_with_stock(self, include_zero_stock: bool = False) -> list:
        ...
        """
        logger.info("📋 Obteniendo CCODs de la base de datos...")
    ...
    async def run_complete_sync(
        self, include_zero_stock: bool = False, batch_size: int = 10, max_ccods: int = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for i in range(0, len(all_ccods), batch_size):
                ...
                logger.info(
                    f"📊 Lote {batch_num} completado: {batch_stats['success']} éxitos, {batch_stats['errors']} errores"
                )
            ...
            success_rate = (
                (self.stats["total_products_created"] / self.stats["total_ccods"] * 100)
                if self.stats["total_ccods"] > 0
                else 0
            )
