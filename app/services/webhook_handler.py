...
class WebhookProcessor:
    ...
    async def process_webhook(
        self, topic: str, payload: Dict[str, Any], webhook_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.error_aggregator.add_error(
                e, {"topic": topic, "webhook_id": webhook_id, "payload_keys": list(payload.keys()) if payload else []}
            )
    ...
    async def _handle_product_create(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._update_local_product_sync_status(product.id, "created", {"created_at": product.createdAt})
            ...
            raise AppException(
                message=f"Failed to process product creation: {str(e)}", details={"payload_id": payload.get("id")}
            ) from e
    ...
    async def _handle_product_update(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._update_local_product_sync_status(product.id, "updated", {"updated_at": product.updatedAt})
            ...
            raise AppException(
                message=f"Failed to process product update: {str(e)}", details={"payload_id": payload.get("id")}
            ) from e
        ...
        try:
            ...
            if not product_id:
                raise ValidationException(
                    message="Product ID missing in delete webhook", field="id", invalid_value=payload.get("id")
                )
            ...
            await self._update_local_product_sync_status(
                product_id, "deleted", {"deleted_at": datetime.now(timezone.utc).isoformat()}
            )

            return {"action": "product_deleted", "product_id": product_id, "sync_needed": False}

        except Exception as e:
            raise AppException(
                message=f"Failed to process product deletion: {str(e)}", details={"payload_id": payload.get("id")}
            ) from e
    ...
    async def _handle_order_create(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise AppException(
                message=f"Failed to process order creation: {str(e)}", details={"payload_id": payload.get("id")}
            ) from e
    ...
    async def _handle_order_update(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise AppException(
                message=f"Failed to process order update: {str(e)}", details={"payload_id": payload.get("id")}
            ) from e
    ...
    async def _handle_order_fulfilled(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise AppException(
                message=f"Failed to process order fulfillment: {str(e)}", details={"payload_id": payload.get("id")}
            ) from e
    ...
    async def _handle_inventory_update(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise AppException(
                message=f"Failed to process inventory update: {str(e)}", details={"payload": payload}
            ) from e
    ...
    async def _handle_app_uninstall(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"action": "app_uninstalled", "shop_domain": shop_domain, "cleanup_initiated": True}

        except Exception as e:
            raise AppException(
                message=f"Failed to process app uninstall: {str(e)}", details={"payload": payload}
            ) from e
    ...
    def _validate_product_payload(self, payload: Dict[str, Any]) -> ShopifyProduct:
        ...
        try:
                    ...
                    variants.append(
                        {
                            ...
                            "compareAtPrice": (
                                str(variant.get("compare_at_price")) if variant.get("compare_at_price") else None
                            ),
            ...
            raise ValidationException(
                message=f"Invalid product payload: {str(e)}", field="payload", invalid_value=str(payload)[:100]
            ) from e
    ...
    def _validate_order_payload(self, payload: Dict[str, Any]) -> ShopifyOrder:
        ...
        try:
            ...
            raise ValidationException(
                message=f"Invalid order payload: {str(e)}", field="payload", invalid_value=str(payload)[:100]
            ) from e
    ...
    async def _update_local_product_sync_status(
        self, product_id: str, status: str, metadata: Optional[Dict[str, Any]] = None
    ):
        """
    ...
    async def _sync_order_to_rms(self, order: ShopifyOrder, raw_payload: Dict[str, Any]):
        ...
        try:
            ...
            self.error_aggregator.add_error(e, {"order_id": order.id, "order_number": order.name})
