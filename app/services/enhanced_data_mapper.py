...
class EnhancedDataMapper:
    ...
    async def _ensure_metafield_definitions(self) -> None:
        ...
        """
        try:
            for _, definition in self.taxonomy_mapper.METAFIELD_DEFINITIONS.items():
    ...
    async def map_rms_item_to_shopify_product(self, rms_item: RMSViewItem) -> Dict[str, Any]:
        ...
        try:
            ...
            taxonomy_mapping = self.taxonomy_mapper.get_taxonomy_mapping(
                rms_item.familia or "", rms_item.categoria or ""
            )
            ...
            clean_title = rms_item.description or f"Producto {rms_item.ccod}"
            product_data = {
                "title": clean_title,
                "descriptionHtml": clean_title,  # Descripción igual al título, sin HTML
            ...
            logger.debug(
                f"Mapped RMS item {rms_item.ccod} to Shopify product"
                f" with taxonomy: {taxonomy_mapping.shopify_product_type}"
            )
            return product_data
    ...
    async def _resolve_taxonomy_id(self, taxonomy_mapping: TaxonomyMapping) -> Optional[str]:
        ...
        try:
            ...
            search_terms = self.taxonomy_mapper.get_search_terms_for_taxonomy_resolution(
                taxonomy_mapping.rms_familia, taxonomy_mapping.rms_categoria
            )
    ...
    def _generate_tags(self, rms_item: RMSViewItem, taxonomy_mapping: TaxonomyMapping) -> List[str]:
        ...
        tags.extend(["RMS-Sync", f"RMS-CCOD-{rms_item.ccod}", taxonomy_mapping.shopify_product_type])
    ...
    def _generate_variants(self, rms_item: RMSViewItem) -> List[Dict[str, Any]]:
        ...
        if talla_original and talla_original != talla_normalizada:
            variant_metafields.append(
                {"namespace": "rms", "key": "original_size", "type": "single_line_text_field", "value": talla_original}
            )
    ...
    def _generate_product_options(self, rms_item: RMSViewItem) -> List[Dict[str, Any]]:
        ...
        if talla_normalizada:
            options.append({"name": "Size", "values": [talla_normalizada]})

        # Opción de color
        if rms_item.color:
            options.append({"name": "Color", "values": [rms_item.color]})
    ...
    async def validate_product_mapping(self, rms_item: RMSViewItem) -> Dict[str, Any]:
        ...
        """
        try:
            validation_result = {"valid": True, "warnings": [], "errors": [], "mapping_info": {}}
            ...
            if rms_item.talla:
                ...
                if talla_original != talla_normalizada:
                    validation_result["warnings"].append(
                        f"Size normalized: '{talla_original}' -> '{talla_normalizada}'"
                    )
            ...
            return {"valid": False, "errors": [f"Validation error: {str(e)}"], "warnings": [], "mapping_info": {}}
