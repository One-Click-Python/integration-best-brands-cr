...
class ShopifyGraphQLClient:
    ...
    async def get_collections(self, limit: int = 250, cursor: Optional[str] = None) -> Dict[str, Any]:
        """
        Obtiene colecciones de Shopify con paginación.

        Args:
            limit: Número de colecciones por página (max 250)
            cursor: Cursor para paginación

        Returns:
            Dict con colecciones y información de paginación
        """
        try:
            variables = {"first": min(limit, 250)}
            if cursor:
            ...
            collections = []
            for edge in result.get("collections", {}).get("edges", []):
                collections.append(edge.get("node"))

            page_info = result.get("collections", {}).get("pageInfo", {})
    ...
    async def get_collection_by_id(self, collection_id: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Colección si existe, None si no se encuentra
        """
        try:
            ...
            if collection:
                logger.info(f"Found collection by ID: {collection.get('title')} ({collection_id})")
                return collection
            else:
            ...
            logger.error(f"Error finding collection by ID {collection_id}: {e}")
            return None

    async def get_collection_by_handle(self, handle: str) -> Optional[Dict[str, Any]]:
        """
        Busca una colección por handle.

        Args:
            handle: Handle de la colección
        ...
        try:
            ...
            if collection:
                logger.info(f"Found collection by handle: {collection.get('title')} ({handle})")
                return collection
            else:
            ...
            logger.error(f"Error finding collection by handle {handle}: {e}")
            return None

    async def get_all_collections(self) -> List[Dict[str, Any]]:
        """
        Obtiene todas las colecciones usando paginación automática.
        ...
        try:
            ...
            logger.info(f"Retrieved {len(all_collections)} total collections across {page_count} pages")
            return all_collections

        except Exception as e:
            logger.error(f"Error getting all collections with pagination: {e}")
            raise
    ...
    async def create_collection(self, collection_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            collection_data: Datos de la colección siguiendo el schema de Shopify

        Returns:
            Colección creada con su ID
        ...
        try:
            ...
            if user_errors:
                error_messages = [f"{e['field']}: {e['message']}" for e in user_errors]
                raise ShopifyAPIException(f"Collection creation failed: {', '.join(error_messages)}")

            collection = collection_result.get("collection")
            if collection:
                logger.info(f"Created collection: {collection['id']} - {collection['title']}")
                return collection
    ...
    async def update_collection(self, collection_id: str, collection_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            collection_id: ID de la colección en Shopify
            collection_data: Datos a actualizar
        ...
        try:
            ...
            if user_errors:
                error_messages = [f"{e['field']}: {e['message']}" for e in user_errors]
                raise ShopifyAPIException(f"Collection update failed: {', '.join(error_messages)}")

            collection = collection_result.get("collection")
            if collection:
                logger.info(f"Updated collection: {collection['id']} - {collection['title']}")
                return collection
    ...
    async def delete_collection(self, collection_id: str) -> bool:
        ...
        try:
            ...
            user_errors = delete_result.get("userErrors", [])

            if user_errors:
                error_messages = [f"{e['field']}: {e['message']}" for e in user_errors]
                raise ShopifyAPIException(f"Collection deletion failed: {', '.join(error_messages)}")
    ...
    async def add_products_to_collection(self, collection_id: str, product_ids: List[str]) -> Dict[str, Any]:
        ...
        Args:
            collection_id: ID de la colección
            product_ids: Lista de IDs de productos a agregar

        Returns:
            Diccionario con la colección actualizada
        """
        try:
            variables = {
                "id": collection_id,
            ...
            if user_errors:
                error_messages = [f"{e['field']}: {e['message']}" for e in user_errors]
                raise ShopifyAPIException(f"Failed to add products to collection: {', '.join(error_messages)}")

            collection = add_result.get("collection")
            if collection:
                logger.info(
                    f"Added {len(product_ids)} products to collection '{collection.get('title')}' "
                    f"(ID: {collection_id})"
                )
                return collection
    ...
    async def remove_products_from_collection(self, collection_id: str, product_ids: List[str]) -> Dict[str, Any]:
        ...
        Args:
            collection_id: ID de la colección
            product_ids: Lista de IDs de productos a remover
        ...
        try:
            ...
            user_errors = remove_result.get("userErrors", [])

            if user_errors:
                error_messages = [f"{e['field']}: {e['message']}" for e in user_errors]
                raise ShopifyAPIException(f"Failed to remove products from collection: {', '.join(error_messages)}")

            collection = remove_result.get("collection")
            if collection:
                logger.info(
                    f"Removed {len(product_ids)} products from collection '{collection.get('title')}' "
                    f"(ID: {collection_id})"
                )
                return collection
