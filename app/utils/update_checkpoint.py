...
class UpdateCheckpointManager:
    ...
    def __init__(self, checkpoint_dir: str = "./checkpoint", checkpoint_filename: str = "checkpoint.json"):
        """
    ...
    def get_last_update_timestamp(self, default_days_back: int = 30) -> datetime:
        ...
        if checkpoint and "last_run_timestamp" in checkpoint:
            try:
                ...
                logger.info(
                    f"🕐 [UPDATE CHECKPOINT] Using timestamp: {last_timestamp.isoformat()} for change detection"
                )
    ...
    def get_checkpoint_status(self) -> Dict[str, Any]:
        ...
        if checkpoint:
            try:
                ...
                return {"exists": True, "error": str(e), "file_path": str(self.checkpoint_file)}

        return {"exists": False, "file_path": str(self.checkpoint_file), "message": "No checkpoint file found"}
